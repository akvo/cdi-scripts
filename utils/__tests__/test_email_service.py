...
def mock_file():
    with patch(
        'builtins.open',
        mock_open(read_data="This is a test file.")
    ) as mock_file:
        yield mock_file
...
def test_send_email_error(mock_mailjet_client, mock_file, mock_html_content):
    mock_mailjet_client.return_value.send.create.return_value.status_code = 400
    mock_mailjet_client.return_value\
        .send.create.return_value.json.return_value = {
...
def test_send_email_missing(mock_mailjet_client, mock_file, mock_html_content):
    mock_mailjet_client.return_value.send.create.return_value.status_code = 404
    mock_mailjet_client.return_value \
        .send.create.return_value.json.return_value = {
