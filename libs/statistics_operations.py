...
class StatisticOperations:
    ...
    def compute_anomalies_from_files(self, files, parameter):
        ...
        try:
            ...
            for f in files:
                ...
                data_set.close()
            month_values = ma.masked_equal(month_values, self.__missing)  # mask out missing data
    ...
    def rank_parameter(self, values):
        ...
        try:
            ...
            for j, base_data in enumerate(values):
                ...
                for i, compare_data in enumerate(values):
            ...
            # mask ranked data #
            masked_ranks = ma.array(ranked_data, mask=masked_values.mask)
            # divide sum by total years #
            ranked_data = np.round(masked_ranks / len(values), 3)
            # find the absolute maximum rank for scaling
            scale = 1 / ma.amax(ma.amax(ranked_data, axis=0))
            # final scaling of the data #
            final_ranks = ranked_data * scale
            return final_ranks
        except ValueError:
            raise
        except Exception:
